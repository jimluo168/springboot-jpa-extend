...
public class BusOnlineDataDeclareService {
    ...
    public BusOnlineDataDeclare insert(BusOnlineDataDeclare declare, MultipartFile file) throws IOException, IllegalAccessException {
        try {
            ...
            List<Map<String, Object>> list = EasyExcel.read(file.getInputStream()).sheet().headRowNumber(2).doReadSync();
            ...
            // 从excel第三行开始读取
            EasyExcel.read(file.getInputStream(), DeclareItemExcelModel.class, new BusOnlineDataDeclareService.ImportDataListener(declareItemService, busTeamService, busRouteService, declare)).sheet().headRowNumber(2).doRead();
            return declare;
        } catch (Exception e) {
